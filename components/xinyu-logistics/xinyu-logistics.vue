	...
	<view class="page">
		...
		<view class="content bgf">
			<view>
				...
				<view class="flex list" :class="{one: index == wlInfo.list.length-1 && wlInfo.delivery_status == 1}" v-for="(item, index) in wlInfo.list"
				 :key="index">
					<view class="time">
					...
					<view class="info flex1">
						<view class="title">{{index == wlInfo.list.length-1 && wlInfo.delivery_status == 1 ? '已签收' : '配送中'}}</view>
