	...
	<view class="page">
		<view class="image-box"  v-for="(item,index) of dataList" :key="index">
			<view class="flex align-items">
				<u-image class="inline-block img" width="150rpx" height="100rpx" :src="vuex_user.idcardHand" mode="scaleToFill"></u-image>
				<u-image class="inline-blockimg" width="150rpx" height="100rpx" :src="vuex_user.idcardHand" mode="scaleToFill"></u-image>
			</view>
			<view class="content">
				<view class="">姓名：{{item.name}}</view>
				<view class="">身份证号码：{{item.idcardNo}}</view>
				<view class="">电话：{{item.phoneNumber}}</view>
				<view class="btn-s-blue" @click="goBack(item)">
					选择
				</view>
			</view>
		</view>
